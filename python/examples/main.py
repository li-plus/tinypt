...
def make_cornell_sphere():
    ...
    left = tinypt.Rectangle(location=(x1, ym, zm), rotation=yz_rot, dimension=(zs, ys),
                            surface=tinypt.Lambertian(texture=(.75, .25, .25)))
    right = tinypt.Rectangle(location=(x2, ym, zm), rotation=yz_rot, dimension=(zs, ys),
                             surface=tinypt.Lambertian(texture=(.25, .25, .75)))
    xz_rot = R.from_euler('x', 90, degrees=True).as_matrix()
    back = tinypt.Rectangle(location=(xm, y2, zm), rotation=xz_rot, dimension=(xs, zs),
                            surface=tinypt.Lambertian(texture=(.75, .75, .75)))
    bottom = tinypt.Rectangle(location=(xm, ym, z1), dimension=(xs, ys),
                              surface=tinypt.Lambertian(texture=(.75, .75, .75)))
    top = tinypt.Rectangle(location=(xm, ym, z2), dimension=(xs, ys),
                           surface=tinypt.Lambertian(texture=(.75, .75, .75)))
    mirror = tinypt.Sphere(radius=16.5, location=(27, -47, 16.5),
                           surface=tinypt.Metal(texture=(1, 1, 1)))
    glass = tinypt.Sphere(location=(73, -78, 16.5), radius=16.5,
                          surface=tinypt.Dielectric(texture=(1, 1, 1), ior=1.5))
    light = tinypt.Circle(radius=18, location=(50, -81.6, 81.6 - 0.01),
                          emission=(12, 12, 12))
    ...
    camera = tinypt.Camera(location=(50, -295.6, 52), rotation=cam_rot,
                           resolution=(1024, 768), fov=math.radians(39.32))
...
def make_cornell_box():
    ...
    camera = tinypt.Camera(location=(0, -3.5, 1), rotation=cam_rot,
                           resolution=(1024, 1024), fov=math.radians(42))
...
def make_breakfast_room():
    ...
    area_light = tinypt.Rectangle(location=(-0.596747, 1.83138, 7.02496),
                                  dimension=(11.3233, 6), emission=(2, 2, 2))
    sunlight_rot = R.from_euler(
        'xyz', [50.3857, -0.883569, 74.8117], degrees=True).as_matrix()
    sunlight = tinypt.DistantLight(
        rotation=sunlight_rot, power=8, angle=math.radians(2.2))

    cam_rot = R.from_euler('x', 90, degrees=True).as_matrix()
    camera = tinypt.Camera(location=(-0.62, -7.59, 1.20), rotation=cam_rot,
                           resolution=(1024, 1024), fov=math.radians(49.1343))
    scene = tinypt.Scene(camera=camera, objects=[
        mesh, area_light], lights=[sunlight])
    return scene
...
def make_living_room():
    ...
    camera = tinypt.Camera(location=(2.2, -7.7, 1.9), rotation=cam_rot,
                           resolution=(1920, 1080), fov=math.radians(67))
    scene = tinypt.Scene(camera=camera, objects=[mesh])
    return scene
...
def make_fireplace_room():
    ...
    camera = tinypt.Camera(location=(5.0, 3.0, 1.1), rotation=cam_rot,
                           resolution=(1920, 1080), fov=math.radians(75))
    scene = tinypt.Scene(camera=camera, objects=[mesh])
    return scene
...
def make_rungholt():
    ...
    camera = tinypt.Camera(location=(270, -209, 169), rotation=cam_rot,
                           resolution=(1920, 1080), fov=math.radians(75))
    scene = tinypt.Scene(camera=camera, objects=[mesh], lights=[
        sunlight], background=background)
    return scene
...
def make_dabrovic_sponza():
    ...
    sunlight_rot = R.from_euler(
        'xyz', [-10, -15, 98], degrees=True).as_matrix()
    sunlight = tinypt.DistantLight(
        rotation=sunlight_rot, power=10, angle=math.radians(20))
    cam_rot = R.from_euler('xyz', (105, 0, 261), degrees=True).as_matrix()
    camera = tinypt.Camera(location=(-12, 1.2, 1.4), rotation=cam_rot,
                           resolution=(1920, 1080), fov=math.radians(81.8))
    scene = tinypt.Scene(camera=camera, objects=[mesh], lights=[
        sunlight], background=tinypt.EnvTexture(texture='#5689BE'))
    return scene
...
def make_salle_de_bain():
    ...
    camera = tinypt.Camera(location=(7.8, -39, 15.5), rotation=cam_rot,
                           resolution=(1080, 1080), fov=math.radians(60))
    scene = tinypt.Scene(camera=camera, objects=[mesh])
    return scene
...
def main():
    ...
    parser.add_argument('--device', type=str, default='cpu',
                        choices=['cpu', 'cuda'])
    parser.add_argument('--scene', type=str, default='cornell_sphere', choices=[
        'cornell_sphere', 'cornell_box', 'breakfast_room', 'living_room',
        'fireplace_room', 'rungholt', 'dabrovic_sponza', 'salle_de_bain'])
    ...
    print(
        f'[Path Tracing] ({args.num_samples} spp) Time: {time.time() - start:.2f}s')
